...
class PermissionService
{
   ...
   public static function allPermissionsGrouped(): array
    ...
    public static function usersWithAccessToProject(Project $project): Collection
    {
        ...
        // Ensure clientCompany exists before accessing clients
        $owners = $project->clientCompany?->clients->load('roles:id,name')
            ->map(fn ($user) => [...$user->toArray(), 'reason' => 'company owner']) ?? collect();

        $givenAccess = $project->users->load('roles:id,name')
